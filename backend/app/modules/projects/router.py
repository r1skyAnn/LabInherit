...
def _build_out(project: "Project") -> ProjectOut:
    return ProjectOut(
        id=project.id,
        title=project.title,
...
async def create_project(
    ...
    if user.status == UserStatus.GRADUATED.value:
        from app.core.exceptions import PermissionDeniedError
        raise PermissionDeniedError("已毕业成员不能新建项目")
    project = await service.create_project(
        db,
        created_by=user.id,
        data=payload.model_dump(exclude_unset=True),
    )
    return _build_out(project)
...
async def list_projects(
    ...
    return ProjectListResponse(
        items=[_build_out(p) for p in projects],
        total=total,
    )
